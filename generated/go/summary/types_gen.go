...
func (resource *SummarySummaryResults) UnmarshalJSONStrict(raw []byte) error {
	...
	// Field "passed"
	if fields["passed"] != nil {
		if string(fields["passed"]) != "null" {
			if err := json.Unmarshal(fields["passed"], &resource.Passed); err != nil {
				errs = append(errs, cog.MakeBuildErrors("passed", err)...)
			}
		} else {
			errs = append(errs, cog.MakeBuildErrors("passed", errors.New("required field is null"))...)

		}
		delete(fields, "passed")
	} else {
		errs = append(errs, cog.MakeBuildErrors("passed", errors.New("required field is missing from input"))...)
	}
...
func (resource SummarySummaryResults) Equals(other SummarySummaryResults) bool {
	...
	if resource.Passed != other.Passed {
		return false
	}
